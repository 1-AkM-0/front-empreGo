      ...
      <div className="p-6 flex flex-col md:flex-row gap-5 justify-between items-start md:items-center">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-0 border-slate-800/50">
          <div className="relative flex-1 md:flex-none min-w-37.5">
            <select value={job.status} onChange={(e) => handleStatusChange(job.application_id, e.target.value)} className={`w-full appearance-none pl-4 pr-10 py-2.5 rounded-xl text-sm font-medium border cursor-pointer outline-none transition-colors ${getStatusColor(job.status)}`}>
          ...
          <a href={job.job_link} target="_blank" rel="noreferrer" className="bg-[#1E2433] hover:bg-[#252C3D] text-slate-300 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors border border-slate-700/50 flex items-center justify-center gap-2">
